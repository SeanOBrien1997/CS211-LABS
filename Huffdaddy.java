...
public class Huffdaddy {
	public static void main(String args[]) throws Exception {
		...
		while (PQ.size() > 1) { // while there are two or more Trees left in the forest
			...
			// get trees to be combined
			Tree first = PQ.poll();
			Tree second = PQ.poll();
			
			Tree t = new Tree(); // create new tree
			t.root = new Node(); // default val
		...
		for(int i = 0; i < sentence.length();i++) {
			s+=(HuffmanTree.getCode(sentence.charAt(i))); // append each chars code to string
		}
